...
export default function Levels({ user, levels = [] }: { user: User; levels?: Level[] }) {
    ...
    const submit = () => {
        router.post(route('registrar.academic.levels.store'), form, {
            preserveScroll: true,
    ...
    const update = (level: Level) => {
        router.put(route('registrar.academic.levels.update', level.id), {
            key: level.key,
            name: level.name,
    ...
    const destroyLevel = (level: Level) => {
        if (confirm(`Delete ${level.name}?`)) {
            router.delete(route('registrar.academic.levels.destroy', level.id), { preserveScroll: true });
        }
    };
