...
export default function Programs({ user, academicLevels = [], departments = [], tracks = [], formErrors }: { user: User; academicLevels?: AcademicLevel[]; departments?: Department[]; tracks?: Track[]; formErrors?: Record<string, string> }) {
    ...
    const destroyTrack = (track: Track) => {
        if (confirm(`Delete track ${track.name}?`)) {
            router.delete(route('registrar.academic.tracks.destroy', track.id), {
                preserveScroll: true,
                onSuccess: () => {
    ...
    const destroyStrand = (strand: Strand) => {
        if (confirm(`Delete strand ${strand.name}?`)) {
            router.delete(route('registrar.academic.strands.destroy', strand.id), {
                preserveScroll: true,
                onSuccess: () => {
    ...
    const destroyDepartment = (dept: Department) => {
        if (confirm(`Delete department ${dept.name}?`)) {
            router.delete(route('registrar.academic.departments.destroy', dept.id), {
                preserveScroll: true,
                onSuccess: () => {
    ...
    const destroyCourse = (course: Course) => {
        if (confirm(`Delete course ${course.name}?`)) {
            router.delete(route('registrar.academic.courses.destroy', course.id), {
                preserveScroll: true,
                onSuccess: () => {
                                        ...
                                        <Dialog open={deptModal} onOpenChange={setDeptModal}>
                                            ...
                                            <DialogContent>
                                                <DialogHeader>
                                                    ...
                                                    <DialogDescription>
                                                        Create a new department for organizing college courses.
                                                    </DialogDescription>
                                                </DialogHeader>
                                                <div className="space-y-3">
                                                    <div>
                                        ...
                                        <Dialog open={courseModal} onOpenChange={setCourseModal}>
                                            ...
                                            <DialogContent>
                                                <DialogHeader>
                                                    ...
                                                    <DialogDescription>
                                                        Create a new course under a specific department.
                                                    </DialogDescription>
                                                </DialogHeader>
                                                <div className="space-y-3">
                                                    <div>
                        ...
                        <Dialog open={!!editStrand} onOpenChange={(open) => !open && setEditStrand(null)}>
                            <DialogContent>
                                <DialogHeader>
                                    ...
                                    <DialogDescription>
                                        Update the strand information below.
                                    </DialogDescription>
                                </DialogHeader>
                                <div className="space-y-3">
                                    <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                                    ...
                                    <div>
                                        <Label>Track *</Label>
                                        <Select value={editStrand.track_id?.toString() || 'none'} onValueChange={(value) => setEditStrand({ ...editStrand, track_id: value === 'none' ? undefined : Number(value) })}>
                                            <SelectTrigger>
                                                <SelectValue placeholder="Select a track" />
                                            </SelectTrigger>
                                            <SelectContent>
