...
const Chairpersons: React.FC<Props> = ({ chairpersons }) => {
    ...
    // Match Admin schema: is_active is boolean but our form util may infer literal true; force to boolean
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingChairperson) {
            put(`/registrar/users/chairpersons/${editingChairperson.id}`, {
                onSuccess: () => {
            ...
            post('/registrar/users/chairpersons', {
                onSuccess: () => {
                    setShowCreateModal(false);
                    reset();
                }
    ...
    const handleEdit = (chairperson: Chairperson) => {
        ...
        setData('contact_number', chairperson.contact_number || '');
        setData('department', chairperson.department || '');
        setData('specialization', chairperson.specialization || '');
        setData('is_active', !!chairperson.is_active as true);
    ...
    const handleDelete = (chairperson: Chairperson) => {
        if (confirm(`Are you sure you want to delete ${chairperson.name}? This action cannot be undone.`)) {
            router.delete(`/registrar/users/chairpersons/${chairperson.id}`);
        }
    };
    ...
    const filteredChairpersons = chairpersons.filter(chairperson =>
        chairperson.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        chairperson.email.toLowerCase().includes(searchTerm.toLowerCase()) ||
        (chairperson.department && chairperson.department.toLowerCase().includes(searchTerm.toLowerCase()))
    );

    return (
        <RegistrarLayout>
            <Head title="Chairpersons - Registrar" />
            
            {/* Header */}
            <div className="mb-8">
                <h1 className="text-3xl font-bold text-gray-900">Chairpersons</h1>
                <p className="text-gray-600 mt-2">Manage chairperson accounts and information</p>
            </div>
            ...
            <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:space-x-3 space-y-3 sm:space-y-0">
                ...
                <a
                    href="/registrar/users/upload?type=chairperson"
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200">
                <div className="px-6 py-4 border-b border-gray-200">
                    <h3 className="text-lg font-semibold text-gray-900">Chairpersons List</h3>
                </div>
                
                {filteredChairpersons.length === 0 ? (
                    <div className="p-6 text-center text-gray-500">
                        {searchTerm ? 'No chairpersons found matching your search.' : 'No chairpersons found. Create your first chairperson to get started.'}
                    </div>
                ) : (
                                ...
                                {filteredChairpersons.map((chairperson) => (
                                    <tr key={chairperson.id} className="hover:bg-gray-50">
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            ...
                                            <div className="text-xs text-gray-500">{chairperson.email}</div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="text-sm text-gray-900">
                                                {chairperson.contact_number || 'N/A'}
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="text-sm text-gray-900">
                                                {chairperson.department || 'N/A'}
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="text-sm text-gray-900">
                                                {chairperson.specialization || 'N/A'}
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                                                ...
                                                {chairperson.is_active ? 'Active' : 'Inactive'}
                                            </span>
                                        </td>
