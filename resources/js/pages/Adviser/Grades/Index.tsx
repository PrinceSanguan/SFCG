...
interface AssignedSubject {
    id: number;
    subject: { id: number; name: string; code: string; course?: { id: number; name: string; code: string } };
    academicLevel: { id: number; name: string; key: string };
    gradingPeriod?: { id: number; name: string };
    school_year: string;
    is_active: boolean;
    enrolled_students: Array<{ id: number; student: { id: number; name: string; email: string }; semester?: string; is_active: boolean; school_year: string }>;
...
export default function AdviserGradesIndex({ user, grades, assignedSubjects }: IndexProps) {
    ...
    const getStudentsForSubject = (subjectId: string) => {
        ...
        return subjectAssignment.enrolled_students.map(enrollment => {
            ...
            return {
                ...
                gradingPeriod: subjectAssignment.gradingPeriod ? { name: subjectAssignment.gradingPeriod.name } : undefined,
                ...
                <main className="flex-1 overflow-y-auto bg-gray-100 p-4 md:p-6 dark:bg-gray-900">
                    <div className="flex flex-col gap-6">
                        <div className="flex flex-col gap-2">
                            ...
                            <p className="text-gray-500 dark:text-gray-400">View and manage student grades for your advisory subjects.</p>
                        </div>
                        ...
                        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                                <div className="relative">
                                    ...
                                    <Input placeholder="Search students or subjects..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10 w-full sm:w-64" />
                                </div>
                            </div>
                            <div className="flex gap-2">
                                <Link href={route('adviser.grades.create')} className={!selectedSubject ? 'pointer-events-none' : ''}>
                                    <Button className="flex items-center gap-2" disabled={!selectedSubject}>
                                        <Plus className="h-4 w-4" />
                                        Input Grade
                                    </Button>
                                </Link>
                                <Link href={route('adviser.grades.upload')}>
                        ...
                        <Card>
                            <CardHeader>
                                ...
                                <p className="text-sm text-muted-foreground">Choose a subject to view and manage student grades</p>
                            </CardHeader>
                            <CardContent>
                                {availableSubjects.length > 0 ? (
                                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                                        {availableSubjects.map(subject => (
                                            <div key={subject.id} className={`p-4 border rounded-lg cursor-pointer transition-colors ${selectedSubject === subject.id ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50 dark:border-gray-700 dark:hover:border-gray-600 dark:hover:bg-gray-800/50'}`} onClick={() => setSelectedSubject(selectedSubject === subject.id ? '' : subject.id)}>
                                                <div className="flex items-center justify-between">
                                                    <div>
                                                        ...
                                                        <p className="text-xs text-gray-400 dark:text-gray-500">{subject.academicLevel} • {subject.schoolYear}</p>
                                                    </div>
                                                    <div className={`w-3 h-3 rounded-full ${selectedSubject === subject.id ? 'bg-blue-500' : 'bg-gray-300 dark:bg-gray-600'}`} />
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    <div className="text-center py-8">
                                        ...
                                        <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">No Subjects Assigned</h3>
                                        <p className="text-gray-500 dark:text-gray-400">You haven't been assigned to any subjects yet.</p>
                                    </div>
                                )}
                            </CardContent>
                        </Card>
                            ...
                            <Card>
                                <CardHeader>
                                    <CardTitle className="flex items-center justify-between">
                                        ...
                                        <span className="text-sm text-gray-500 dark:text-gray-400">{filteredStudents.length} students</span>
                                    </CardTitle>
                                </CardHeader>
                                <CardContent>
                                    {filteredStudents.length > 0 ? (
                                                ...
                                                <tbody>
                                                    {filteredStudents.map((student) => (
                                                        <tr key={student.id} className="border-b hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                                            ...
                                                            <td className="p-3">
                                                                <span className={`text-sm ${getGradeStatus(student.latestGrade, student.academicLevel.key) === 'No Grade' ? 'text-gray-600 dark:text-gray-400' : ['Superior', 'Very Good', 'Good', 'Satisfactory', 'Fair', 'Outstanding'].includes(getGradeStatus(student.latestGrade, student.academicLevel.key)) ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                                                            ...
                                                            <td className="p-3">
                                                                <div className="flex gap-2">
                                                                    ...
                                                                    <Button size="sm" onClick={(e) => { e.stopPropagation(); handleInputGrade(student); }}>
                                        ...
                                        <div className="text-center py-8">
                                            ...
                                            <p className="text-gray-500 dark:text-gray-400">{searchTerm ? 'No students match your search criteria.' : 'No students are enrolled in this subject.'}</p>
                                        </div>
                                    )}
                                </CardContent>
                            </Card>
