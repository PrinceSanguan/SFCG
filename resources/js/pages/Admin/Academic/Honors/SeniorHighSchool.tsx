                                            ...
                                            return (
                                                <>
                                                    <div className="flex items-center justify-between pb-4 border-b">
                                                        ...
                                                        <Button
                                                            onClick={() => {
                                                                console.log('[SHS Submit] Starting submission process...');
                                                                console.log('[SHS Submit] Qualified students count:', actuallyQualified.length);
                                                                console.log('[SHS Submit] Current school year:', currentSchoolYear);
                                                                console.log('[SHS Submit] Honor results count:', honorResults.length);
                                                                ...
                                                                console.log('[SHS Submit] Total qualified students:', actuallyQualified.length);
                                                                console.log('[SHS Submit] Already submitted count:', alreadySubmittedCount);
                                                                console.log('[SHS Submit] Students to submit count:', studentsToSubmit.length);
                                                                console.log('[SHS Submit] Student names to submit:', studentsToSubmit.map((s: any) => s.student.name));

                                                                if (alreadySubmittedCount > 0) {
                                                                    console.log('[SHS Submit] Already submitted students:',
                                                                ...
                                                                if (studentsToSubmit.length === 0) {
                                                                    console.log('[SHS Submit] All students already submitted - blocking submission');
                                                                ...
                                                                if (confirm(`Submit ${studentsToSubmit.length} qualified student(s) for principal approval?`)) {
                                                                    console.log('[SHS Submit] User confirmed. Sending data...');
                                                                    console.log('[SHS Submit] Submitting students:', studentsToSubmit.map((s: any) => ({ id: s.student.id, name: s.student.name })));

                                                        router.post(route('admin.academic.honors.senior-high-school.generate-results'), {
                                                            school_year: currentSchoolYear,
                                                        }, {
                                                            onSuccess: (response) => {
                                                                console.log('[SHS Submit] Success response:', response);
                                                                addToast(`Successfully submitted ${studentsToSubmit.length} student(s) for approval!`, 'success');
                                                            },
                                                            onError: (errors) => {
                                                                console.error('[SHS Submit] Error response:', errors);
                                                                addToast('Failed to submit students for approval. Please try again.', 'error');
                                                            }
                                                        });
                                                    } else {
                                                        console.log('[SHS Submit] User cancelled submission');
                                                    }
                                                }}
                                                className="flex items-center gap-2"
                                            >
