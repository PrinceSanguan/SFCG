...
const CreateCollege: React.FC<Props> = ({ students, subjects, academicPeriods, instructors }) => {
    const { data, setData, post, processing, errors } = useForm({
        ...
        section: '',
        quarterly_grades: [
            { quarter: 1, grade: '', weight: 50, label: 'Midterm', semester: 1 },
            { quarter: 2, grade: '', weight: 50, label: 'Pre-Final', semester: 1 },
            { quarter: 3, grade: '', weight: 50, label: 'Midterm', semester: 2 },
            { quarter: 4, grade: '', weight: 50, label: 'Pre-Final', semester: 2 }
        ],
        final_grade: '',
        remarks: '',
        status: 'draft'
    ...
    const handleQuarterlyGradeChange = (index: number, field: 'grade' | 'weight', value: string) => {
        ...
        // Calculate final grade for College (semester-based: Midterm + Pre-Final / 2)
    ...
    return (
        <>
            <Head title="Create College Grade" />
            <AdminLayout>
                <div className="mb-8">
                    <div className="flex items-center justify-between">
                        <div>
                            <h1 className="text-3xl font-bold text-gray-900">Create College Grade</h1>
                            <p className="text-gray-600 mt-2">Add new grades for College/University students</p>
                        </div>
                        <Link
                            href="/admin/grading/college"
                            className="bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-gray-700 transition-colors"
                        >
                            ← Back to College Grades
                        </Link>
                    </div>
                </div>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-6">
                                        ...
                                        {students.map((student) => (
                                            <option key={student.id} value={student.id}>
                                                ...
                                                {student.student_profile.college_course && (
                                                    <span> - {student.student_profile.college_course.name}</span>
                                                )}
                                            </option>
                                        ))}
                                    </select>
                            ...
                            {selectedStudent && (
                                <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                                    <h3 className="text-sm font-medium text-orange-800 mb-2">Student Information</h3>
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                        <div>
                                            <span className="text-orange-700 font-medium">Name:</span>
                                            <p className="text-orange-800">{selectedStudent.name}</p>
                                        </div>
                                        <div>
                                            <span className="text-orange-700 font-medium">Student ID:</span>
                                            <p className="text-orange-800">{selectedStudent.student_profile.student_id}</p>
                                        </div>
                                        <div>
                                            <span className="text-orange-700 font-medium">Grade Level:</span>
                                            <p className="text-orange-800">Grade {selectedStudent.student_profile.grade_level}</p>
                                        </div>
                                        <div>
                                            <span className="text-orange-700 font-medium">Section:</span>
                                            <p className="text-orange-800">{selectedStudent.student_profile.section}</p>
                                        </div>
                                        {selectedStudent.student_profile.college_course && (
                                            <div className="md:col-span-2">
                                                <span className="text-orange-700 font-medium">Course:</span>
                                                <p className="text-orange-800">{selectedStudent.student_profile.college_course.name} ({selectedStudent.student_profile.college_course.code})</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    {/* 1st Semester */}
                                    <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                                        <h4 className="text-sm font-medium text-orange-800 mb-3">1st Semester</h4>
                                                        ...
                                                        <div className="space-y-2">
                                                            <div>
                                                                ...
                                                                <input
                                                                    ...
                                                                    className="w-full border border-gray-300 rounded-md px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500"
                                                                    placeholder="0.00"
                                                                />
                                                            </div>
                                                            <div>
                                    ...
                                    {/* 2nd Semester */}
                                    <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                                        <h4 className="text-sm font-medium text-orange-800 mb-3">2nd Semester</h4>
                                        <div className="grid grid-cols-1 gap-4">
                            ...
                            <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                                <div className="flex items-center justify-between">
                                    <div>
                                        <h3 className="text-lg font-medium text-orange-800">Final Grade</h3>
                                        <p className="text-sm text-orange-600">Automatically calculated based on quarterly grades</p>
                                    </div>
                            ...
                            <div className="flex justify-end space-x-4 pt-6 border-t border-gray-200">
                                ...
                                <button
                                    type="submit"
                                    disabled={processing}
                                    className="px-6 py-2 bg-orange-600 text-white rounded-md text-sm font-medium hover:bg-orange-700 disabled:opacity-50 transition-colors"
                                >
