...
const CreateJuniorHigh: React.FC<Props> = ({ students, subjects, academicPeriods, instructors }) => {
    ...
    const handleQuarterlyGradeChange = (index: number, field: 'grade' | 'weight', value: string) => {
        ...
        const totalWeight = newQuarterlyGrades.reduce((sum, q) => sum + (Number(q.weight) || 0), 0);
        const weightedSum = newQuarterlyGrades.reduce((sum, q) => sum + ((Number(q.grade) || 0) * (Number(q.weight) || 0)), 0);
    ...
    return (
        <>
            <Head title="Create Junior High School Grade" />
            <AdminLayout>
                <div className="mb-8">
                    <div className="flex items-center justify-between">
                        <div>
                            <h1 className="text-3xl font-bold text-gray-900">Create Junior High School Grade</h1>
                            <p className="text-gray-600 mt-2">Add new grades for Junior High School students (Grades 7-10)</p>
                        </div>
                        <Link
                            href="/admin/grading/junior-high"
                            className="bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-gray-700 transition-colors"
                        >
                            ← Back to Junior High Grades
                        </Link>
                    </div>
                </div>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    >
                                        ...
                                        {/* Use student's existing section or common A-F fallback if none */}
                            ...
                            {selectedStudent && (
                                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                                    <h3 className="text-sm font-medium text-blue-800 mb-2">Student Information</h3>
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                        <div>
                                            <span className="text-blue-700 font-medium">Name:</span>
                                            <p className="text-blue-800">{selectedStudent.name}</p>
                                        </div>
                                        <div>
                                            <span className="text-blue-700 font-medium">Student ID:</span>
                                            <p className="text-blue-800">{selectedStudent.student_profile.student_id}</p>
                                        </div>
                                        <div>
                                            <span className="text-blue-700 font-medium">Grade Level:</span>
                                            <p className="text-blue-800">Grade {selectedStudent.student_profile.grade_level}</p>
                                        </div>
                                        <div>
                                            <span className="text-blue-700 font-medium">Section:</span>
                                            <p className="text-blue-800">{selectedStudent.student_profile.section}</p>
                                        </div>
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                                <div className="flex items-center justify-between">
                                    <div>
                                        <h3 className="text-lg font-medium text-blue-800">Final Grade</h3>
                                        <p className="text-sm text-blue-600">Automatically calculated based on quarterly grades</p>
                                    </div>
...
export default CreateJuniorHigh; 